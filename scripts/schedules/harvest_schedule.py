...
def main(season: int, week: str, outdir: str):
    """
    Harvests the week's schedule. Prefers Odds API (commence_time) if ODDSAPI__KEY is present.
    Falls back to a deterministic demo schedule if not.
    """
    s = get_settings()
    ...
    if not rows:
        # Fallback demo: 1pm ET (17:00Z) Sun game
