...
class ScheduleClientConfig:
    # We reuse the same Odds API endpoint because it includes commence_time per game.
    base_url: str = "https://api.the-odds-api.com/v4/sports/americanfootball_nfl/odds"
    api_key: str | None = None
    region: str = "us"
    markets: str = "h2h"            # we only need 1 market to get game objects w/ commence_time
...
class ScheduleClient:
    ...
    def _parse(self, payload, *, season: int, week: int) -> List[Game]:
        # payload shape: list[{id, home_team, away_team, commence_time, bookmakers:[...]}]
        out: List[Game] = []
        ...
        for g in payload:
            ...
            kt = g.get("commence_time")  # ISO string with Z
            if not (gid and home and away and kt): 
                continue
            out.append(Game(game_id=gid, season=season, week=week, home=home, away=away, kickoff_utc_iso=kt, venue=None))
        return out

def harvest_schedule_from_oddsapi(*, api_key: str | None, season: int, week: int) -> Tuple[List[Game], ServiceStatus]:
