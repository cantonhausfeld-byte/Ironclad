...
def size_portfolio(picks: pd.DataFrame, *, bankroll_units: float = 100.0, kelly_scale: float = 0.25,
                   max_per_bet_u: float = 3.0, max_per_game_u: float = 10.0, max_total_u: float = 25.0) -> pd.DataFrame:
    """
    ...
    # enforce per-game by proportional scaling within each game
    out_stake = []
    ...
    # (Optional) simple exposure heuristic: downweight lower grades slightly
